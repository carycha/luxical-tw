...
class Embedder:
    ...
    def from_components(
        ...
        """
        from luxical_tw.tokenization import create_optimized_arrow_tokenizer
        from luxical_tw.ngrams import build_ngram_hash_to_idx_map
        ...
        from luxical_tw.ngrams import fnv1a_hash_array_to_int64
    ...
    def load(cls, path: str | Path) -> Embedder:
        ...
        Note: Global settings like user dictionaries should be configured via luxical_tw.init().
        """
        path = Path(path).resolve()
    ...
    def tokenize(
        ...
        """
        Tokenizes text using the internal tokenizer.
        Defaults to ChineseLexicalTokenizer behavior if applicable.
        """
        # 為了效能與一致性，我們在這裡判斷是否使用中文分詞引擎
        # 在 Luxical-TW 中，我們全面預設使用中文分詞
        from luxical_tw.chinese_tokenization import ChineseLexicalTokenizer
        
        # 如果 internal tokenizer 是原生 ArrowTokenizer，我們將其封裝進 ChineseLexicalTokenizer
        # 這樣可以享受到繁體中文正規化與 Jieba 分詞的優點
        if not hasattr(self, "_chinese_tokenizer"):
            # 從 tokenizer 狀態重建 vocab
            vocab_dict = json.loads(self.tokenizer.to_str())["model"]["vocab"]
            # 建立快取的分詞器
            object.__setattr__(self, "_chinese_tokenizer", ChineseLexicalTokenizer(vocab=vocab_dict))
        
        # 執行批次分詞
        token_pa = self._chinese_tokenizer.tokenize_batch(pa.array(texts), progress_bar=False)

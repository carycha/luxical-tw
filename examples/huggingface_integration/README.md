...
Utilities for packaging Luxical-TW embedders as Huggingface Transformers models and validating parity with the native Luxical-TW pipeline. While the examples below use the Luxical-TW One checkpoint, the wrapper is generic for any Luxical-TW `Embedder` saved with format version 1.

## Layout

- `luxical_hf_wrapper.py` — Generic Huggingface `PreTrainedModel` wrapper around the Luxical-TW `Embedder` plus an all-in-one CLI for export and verification.
...
Produce a Huggingface directory (defaults shown). The CLI accepts any Luxical-TW embedder `.npz`:
...
emb = local_model(["Luxical-TW integrates with Huggingface."]).embeddings
```
